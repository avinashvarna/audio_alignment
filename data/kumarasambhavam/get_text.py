...
# https://raw.githubusercontent.com/sanskritsahitya-com/data/main/kumarasambhavam/kumarasambhavam.json


if __name__ == "__main__":
    _start = datetime.datetime.now()

    # Even though there are 17 sargas of audio, the JSON only seems to contain 8 sargas
    num_sargas = 8
    mp3_stem = "Kumarasambhavam"
    json_path = "kumarasambhavam.json"
    ...
    with open("text_audio_map.csv", "w") as f:
        for idx in range(num_sargas):
            s = f"{idx+1:02d}"
            f.write(f"text/{s}.txt,audio/{mp3_stem}-{s}.mp3\n")
